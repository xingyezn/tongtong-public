...
- 每台设备可独立设置模型、语言、音色、人物设定、对话连续时长和 VAD 参数。
- 对话记录改为 GPT 风格：一次会话是一条记录，内部按时间连续保存独立的用户消息和 AI 消息，不再按问答轮次显示卡片。
...
- 按用户和设备保存最近 20 个完整问答，设备或服务端重启后仍可继续当前对话。
- 对话空闲超时可设置为 1～120 分钟，默认 10 分钟。
- 服务端和设备端使用约 240 ms 启动预缓冲，并采用基于截止时间的发帧节拍，减少断音和卡顿。
- 设备 WebSocket 断开后自动重连，使用指数退避，最长等待 30 秒。
- 支持真正的实时打断：打断时同时停止设备播放并取消模型生成；复位待命键单击后打断并继续聆听，双击结束会话；唤醒词自动打断及三项行为都可按设备配置。
...
流式回传生成的语音，并按用户和设备隔离对话状态。会话仅在明确结束或空闲超时后关闭，内部消息作为
一段连续聊天保存。只有用户启用且设备获准使用的长期记忆才会注入系统提示词。模型 WebSocket 可用时会持续复用；如果服务商、
设备或服务端连接重建，则利用 SQLite 中近期的语音转写恢复上下文。AI 回复文字也会随生成过程同步
发送到设备屏幕，不必等整段语音结束。
...
- `GET|POST /api/model?device_id=...` — 每台设备的模型、语言、音色、人物设定和对话超时设置。
