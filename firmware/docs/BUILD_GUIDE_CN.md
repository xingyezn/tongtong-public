...
固件版本号必须以**线上生产环境（8080）固件版本库中的最大版本号**为基准递增，不能使用测试环境（8081/8082）的版本号作为基准。发布或上传前，先在生产环境管理员页面核对当前最大版本，再确定本次版本号。

版本格式为 `主版本.次版本.修订版本`，每一位数字范围为 `0` 到 `9`，可用范围为 `1.0.0` 到 `9.9.9`：

- 正常情况下修订版本加 1，例如 `1.0.4` → `1.0.5`；
- `1.0.9` → `1.1.0`，按三位数字从右向左进位；
- `1.9.9` → `2.0.0`；
- `9.9.9` 已达到上限，不能自动递增，必须先取得用户确认。

版本号必须唯一且连续，不能重复使用或跳过版本。构建文件中的 `firmware\CMakeLists.txt` 的 `PROJECT_VER` 必须与本次发布版本一致；测试构建也应使用已经确定的版本号，避免设备上报版本与固件库记录不一致。
...
构建成功后，固件文件位于 `firmware\build\tongtong.bin`。发布到后端固件库前，先确认当前分支、目标环境和测试后端端口；非 `main` 分支只能使用已确认的测试后端（8081=浩然，8082=浩鑫）。

管理员页面发布流程：

1. 登录对应环境的管理员页面。
2. 在“固件版本库”选择 `firmware\build\tongtong.bin`。
3. 填写唯一的固件版本号和版本描述，点击上传。
4. 核对页面显示的文件大小和 SHA-256；固件库最多保留 20 个版本。
5. 选择在线终端并点击“下发”，通过进度条和日志确认指令发送、文件下载、设备重启和重新上线。

发布流程中是否同步发布固件必须单独取得确认。文档、脚本和示例配置不得写入管理员账号、密码、API Key、设备 Token 或其它真实凭据；凭据只通过服务器环境变量、未跟踪本地配置或管理员页面输入提供。
